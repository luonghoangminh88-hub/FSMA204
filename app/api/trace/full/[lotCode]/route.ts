...
export const GET = withRateLimit(async (request: NextRequest, { params }: { params: Promise<{ lotCode: string }> }) => {
  try {
    ...
    // Call the full chain trace function
    const { data, error } = await supabase.rpc("trace_full_chain", {
      p_lot_code: trimmedLotCode,
      p_organization_id: profile.organization_id,
    })

    if (error) {
      throw new AppError("Failed to trace full chain", ErrorCode.DATABASE_ERROR, 500)
    }

    // Transform data into hierarchical structure
    const backward = data?.filter((d: any) => d.direction === "backward") || []
    const current = data?.find((d: any) => d.direction === "current")
    const forward = data?.filter((d: any) => d.direction === "forward") || []
    ...
    return NextResponse.json({
      ...
      current,
      forward,
      total_chain_length: data?.length || 0,
    })
  } catch (error) {
    return handleError(error, { endpoint: "trace full chain" })
  }
}, RATE_LIMITS.RELAXED)
